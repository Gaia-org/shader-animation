...
val gradientAnimShader = RuntimeShader(
    """
        ...
        vec4 main(vec2 fragCoord) {
            ...
            vec4 col = vec4(0.8 + 0.2 * cos(time*2.0+uv.xxx*2.0+vec3(1,2,4)), 1.0);
            // Output to screen
            return col;
        }
    """
...
val gradientBlendAnimShader = RuntimeShader(
    """
        uniform float2 resolution;
        ...
        vec4 main(vec2 fragCoord) {
            ...
            // Output to screen
            return mix(tex, col, 0.5);
        }
    """
)

val pipelineZoomShader = RuntimeShader(
    """
        uniform float2 resolution;
        uniform shader uContent; 
        uniform float time;
...
//            return ((sin(p) + vec3(2, 5, 12)) / length(p)).xyz1;
            vec4 calColor = vec4(((sin(p) + vec3(2, 5, 12)) / length(p)).xyz, 1.0);
            // Android view中内容
            vec4 tex = vec4(uContent.eval(fragcoord));
            return mix(tex, calColor, 0.5);
//            return calColor;
        }
    """
)
